if __name__ == '__main__':
    ...
    for t in tqdm(np.arange(0,10,dt)):
        ...
        write_channel_voltage(ao.diOnly, 2, x)
        write_channel_voltage(ao.diOnly, 3, y)
        write_channel_voltage(ao.diOnly, 4, x)
        write_channel_voltage(ao.diOnly, 5, y)
        time.sleep(dt)
    write_channel_voltage(ao.diOnly, 0, 0)
    write_channel_voltage(ao.diOnly, 1, 0)
    write_channel_voltage(ao.diOnly, 2, 0)
    write_channel_voltage(ao.diOnly, 3, 0)
    write_channel_voltage(ao.diOnly, 4, 0)
    write_channel_voltage(ao.diOnly, 5, 0)
