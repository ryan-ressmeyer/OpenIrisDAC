...
class GUI:
    ...
    def window_loop(self, open_iris_ip='localhost', verbose=False):
        assert ao.GetDevices() > 0, 'No USB DAC devices found'
        ao.DACSetBoardRange(ao.diOnly, 1)

        with OpenIrisClient(server_address=open_iris_ip) as client:
            ...
            while True:
                ...
                if event == sg.TIMEOUT_EVENT:
                    ...
                    write_channel_voltage(ao.diOnly, 0, out.x)
                    write_channel_voltage(ao.diOnly, 1, out.y)
